...
import org.openmuc.openiec61850.internal.ClientReceiver;
...
import org.openmuc.openiec61850.internal.acse.ClientAcseSap;
...
public final class ClientAssociation {
	...
	private final BlockingQueue<MmsPdu> incomingReports = new LinkedBlockingQueue<MmsPdu>();
	...
	private void handleInitiateResponse(MmsPdu responsePdu, int proposedMaxPduSize,
		...
		if (negotiatedMaxPduSize < ClientSap.MINIMUM_MMS_PDU_SIZE || negotiatedMaxPduSize > proposedMaxPduSize
			...
			acseAssociation.disconnect();
			throw new IOException("Error negotiating paramters");
		}
	 ...
	 * @throws IOException
	 *             if any kind of fatal association error occurred.
	 */
	 ...
	 *             if a ServiceError is returned by the server.
	 * @throws IOException
	 *             if any kind of fatal association error occurs.
	 */
	...
	/**
	 * The client should create the data set first and add it to either the nonpersistent list or to the model. Then it
	...
	public List<ServiceError> setRcbValues(Rcb rcb, boolean setRptId, boolean setDatSet, boolean setOptFlds,
			...
			boolean setResvTms) throws IOException {
	...
	private Report processReport(InformationReport rpt) throws ServiceError {
		...
		// TODO if dataSetRef == null get it from the RCB

		// updating of data set copy - original stays the same
		dataSet = serverModel.getDataSet(dataSetRef).copy();
		int shiftNum = 0;
		...
		// List<BdaReasonForInclusion> reasonCodes = new ArrayList<BdaReasonForInclusion>();
		...
		return new Report(rptId, optFlds, sqNum, subSqNum, moreSegmentsFollow, dataSetRef, bufOvfl, confRev,
				timeOfEntry, entryId, inclusionBitString, null, dataSet);
