...
public final class TConnection {
	...
	TConnection(Socket socket, int maxTPduSizeParam, int messageTimeout, int messageFragmentTimeout) throws IOException {
	...
	public void receive(ByteBuffer tSduBuffer) throws EOFException, SocketTimeoutException, IOException,
			TimeoutException {
		if (!valid || !connected) {
			throw new IOException("TConnection is already closed.");
		}
